...
import (
	...
	local "github.com/edgexfoundry/device-snmp-go/hooks"
)
...
func main() {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("edgex-device-snmp:configure: can't read value of 'debug': %v", err))
		os.Exit(1)
	}
	if status == "true" {
		debug = true
	...
	if err = hooks.Init(debug, "edgex-device-snmp"); err != nil {
		fmt.Println(fmt.Sprintf("edgex-device-snmp:configure: initialization failure: %v", err))
		os.Exit(1)

	}

	log.SetComponentName("configure")
	if err := options.ProcessAppConfig("device-snmp"); err != nil {
	...
	if envJSON != "" {
		hooks.Debug(fmt.Sprintf("edgex-device-snmp:configure: envJSON: %s", envJSON))
		err = hooks.HandleEdgeXConfig("device-snmp", envJSON, local.ConfToEnv)
		if err != nil {
	...
	if autostart == "" {
		hooks.Debug("edgex-device-snmp autostart is NOT set, initializing to 'no'")
		autostart = "no"
	}
	autostart = strings.ToLower(autostart)

	hooks.Debug(fmt.Sprintf("edgex-device-snmp autostart is %s", autostart))
	...
	case "true":
		fallthrough
	case "yes":
		err = cli.Start("device-snmp", true)
		if err != nil {
